...
function ItemDetails({id, nombre, receta, precio, imagen, category, stock}) {
    ...
    return (
      ...
      <h4 className="receta__titulo">{category}</h4>
          ...
          <footer className='ItemFooter'>
                { quantity > 0  ? <Link to='/cart'>Finalizar compra</Link> : <ItemCount stock={stock} onAdd={handleOnAdd} initial={getProduct(id)?.quantity}/>}
            </footer>
      </div>
  </div>
